...
exports.proxy = function(config){  
  config["default"]               = ["http://127.0.0.1:80"];
  config["localhost:4444"]        = ["http://127.0.0.1:80"];
  config["mydomain.org:4444"]     = ["http://127.0.0.1:80"];
  config["www.mydomain.org:4444"] = ["http://127.0.0.1:80"];
  config["ws.mydomain.org:4444"]  = ["http://127.0.0.1:8111"];
  return config;
}
...
exports.limiters = function(config){
  ...
  config["localhost:4444"]     = { unit: "minute", rate:30 };    // 90 requests per minute 
  config["localhost:4444/foo"] = { unit: "minute", rate:20 };    // overrides 90 with 20 
  return config;
}
