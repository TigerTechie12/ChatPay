...
function start(amqp){
  var bullmqproxyServer = http.createServer(function (req, res) {
    ...
    try{
      ...
      var remainingreq = limiter.getTokensRemaining();
      limiter.removeTokens( 1, function(err, remainingRequests){
        res.setHeader('X-Rate-Limit-Remaining',remainingRequests);
        res.setHeader('X-Rate-Limit-Limit',limiter.tokenBucket.bucketSize );
        console.dir(limiter);
        if( remainingreq < 1 ){
          bullmq.log("REJECT => "+host+req.url+" from "+ip+" (remaining:"+remainingreq+")" );
          ...
          if( config.queue[host] != undefined && config.queue[host][req.url] != undefined ){
              ...
              bullmq.log("QUEUED => "+host+req.url+" from "+ip+" (remaining:"+remainingreq+")" );
            ...
            bullmq.log("ACCEPT => "+host+req.url+" from "+ip+" (remaining:"+remainingreq+")" );
            p.web(req, res);
          }
        }
      });
...
bullmq.log("                    |_|");
bullmq.log("listening at port "+ (process.env.PORT || 4444) );
bullmq.log("config:");
bullmq.log( config );
